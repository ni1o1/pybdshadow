...
def cal_sunshine(buildings, day='2022-01-01', roof=False, grids=gpd.GeoDataFrame(), accuracy=1, precision=3600, padding=1800, areaonly=True):
    '''
    ...
    buildings_aeqd = convert_buildings_to_aeqd(buildings, lon, lat)
    buildings_aeqd['area'] = buildings_aeqd['geometry'].area
    buildings_aeqd = buildings_aeqd[['building_id', 'area']]
    ...
    # 计算每个建筑物在每个时间间隔内的阴影面积
    timetable = shadows[['date']].drop_duplicates()
    timetable['flag'] = 1
    buildings_aeqd['flag'] = 1
    timetable_buildings = pd.merge(timetable,buildings_aeqd,on='flag').drop('flag',axis=1)


    shadows_aeqd = convert_buildings_to_aeqd(shadows, lon, lat)
    shadows_aeqd = shadows_aeqd[shadows_aeqd['type'] == 'roof']
    shadows_aeqd['area'] = shadows_aeqd['geometry'].area

    merged_gdf = pd.merge(shadows_aeqd,
        timetable_buildings, 
        how ='right',
        on=['building_id','date'],
          suffixes=('_1', '_2'))
    merged_gdf['area_1'] = merged_gdf['area_1'].fillna(0)

    # 计算面积差并将结果存储在新列 'area_difference' 中
    merged_gdf['light_area'] = merged_gdf['area_2'] - merged_gdf['area_1']
    merged_gdf = calculate_irradiance_gdf(merged_gdf, lat, lon)
    merged_gdf['roof_pv_potential'] = merged_gdf['clearsky_ghi'] * \
        merged_gdf['light_area']
    # 只保留 'building_id', 'roof_pv_potential', 和 'date' 列
    result_data = merged_gdf[['building_id', 'roof_pv_potential', 'date']]
    if areaonly:
        return result_data
    ...
    if accuracy == 'vector':
        if roof:
            ...
            # 额外：增加屋顶面
            buildings.crs = shadows.crs
            shadows = pd.concat([shadows, buildings])
            # return shadows
            shadows = shadows.groupby('height').apply(
                count_overlapping_features).reset_index()
            shadows['count'] -= 1
        else:
